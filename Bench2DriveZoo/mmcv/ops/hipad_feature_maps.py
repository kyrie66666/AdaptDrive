...
def feature_maps_format(feature_maps, inverse=False):
    if inverse:
        ...
        del scale_start_index
        num_cams, num_levels = spatial_shape.shape[:2]

        split_size = (spatial_shape[..., 0] * spatial_shape[..., 1]).cpu().numpy().tolist()
        split_size = [[int(v) for v in row] for row in split_size]

        cam_split = [1]
        cam_split_size = [int(sum(split_size[0]))]
        for i in range(num_cams - 1):
            if not torch.all(spatial_shape[i] == spatial_shape[i + 1]):
                cam_split.append(0)
                cam_split_size.append(0)
            cam_split[-1] += 1
            cam_split_size[-1] += int(sum(split_size[i + 1]))

        mc_feat = [
            x.unflatten(1, (cam_split[i], -1))
            for i, x in enumerate(col_feats.split(tuple(cam_split_size), dim=1))
        ]
        ...
        for i, feat in enumerate(mc_feat):
            feat = list(feat.split(tuple(split_size[shape_index]), dim=2))
            for j, f in enumerate(feat):
                shape_hw = tuple(int(v) for v in spatial_shape[shape_index][j])
                feat[j] = f.unflatten(2, shape_hw)
    ...
    scale_start_index = torch.cat([torch.tensor([0], device=scale_start_index.device), scale_start_index[:-1]])
