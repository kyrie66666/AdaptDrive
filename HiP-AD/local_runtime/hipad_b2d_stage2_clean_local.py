...
# Runtime paths. MMCV substitutes ``{{ fileDirname }}`` with the directory of
# this config before executing it, so these paths do not depend on the process
# working directory or on the machine where the repository was created.
project_dir = __os.path.abspath(__os.path.join("{{ fileDirname }}", __os.pardir))
...
if __asset_root_override:
    ...
    #   <parent>/AdaptDrive/HiP-AD
    #   <parent>/AdaptDrive-assets
