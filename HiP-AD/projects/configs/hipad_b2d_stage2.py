...
# Runtime paths. MMCV substitutes ``{{ fileDirname }}`` with this config's
# directory before execution.
project_dir = __os.path.abspath(
    __os.path.join("{{ fileDirname }}", __os.pardir, __os.pardir)
)
__asset_root_override = __os.environ.get("ADAPTDRIVE_ASSET_ROOT")
if __asset_root_override:
    if not __os.path.isabs(__asset_root_override):
        raise ValueError("ADAPTDRIVE_ASSET_ROOT must be an absolute path")
    adaptdrive_asset_root = __os.path.abspath(__os.path.expanduser(__asset_root_override))
else:
    adaptdrive_asset_root = __os.path.abspath(
        __os.path.join(project_dir, __os.pardir, __os.pardir, "AdaptDrive-assets")
    )
backbone_pretrained_path = __os.path.join(
...
__stage1_checkpoint = __os.environ.get("HIPAD_STAGE1_CKPT")
if __stage1_checkpoint:
    if not __os.path.isabs(__stage1_checkpoint):
        raise ValueError("HIPAD_STAGE1_CKPT must be an absolute path")
    load_from = __os.path.abspath(__os.path.expanduser(__stage1_checkpoint))
else:
    load_from = None
