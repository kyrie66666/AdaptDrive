...
from rl.sim_backend import SimulationBackend, build_simulation_config_from_args
import time
import random
...
def get_weather_id(weather_conditions):
    ...
    weather_xml = os.environ.get("WEATHER_XML", "")
    if not weather_xml:
        leaderboard_root = os.environ.get("LEADERBOARD_ROOT", "")
        if leaderboard_root:
            weather_xml = os.path.join(leaderboard_root, "data", "weather.xml")
        else:
            weather_xml = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "weather.xml")
...
class LeaderboardEvaluator(object):
    ...
    frame_rate = float(os.environ.get("B2D_FRAME_RATE", "10.0"))  # in Hz
    ...
    def _setup_simulation(self, args):
        ...
        """
        self._sim_backend = SimulationBackend(build_simulation_config_from_args(args, frame_rate=self.frame_rate, default_timeout=self.client_timeout))
        self.client, self.client_timeout, self.traffic_manager = self._sim_backend.start()
        self.server = self._sim_backend.server_process
        self.carla_path = self._sim_backend.config.resolve_carla_root()
        args.port = self._sim_backend.config.port
        args.traffic_manager_port = self._sim_backend.config.traffic_manager_port
    ...
    def _reset_world_settings(self):
        ...
        if self._sim_backend is not None:
            self._sim_backend.world = self.world
            self._sim_backend.traffic_manager = self.traffic_manager
            self._sim_backend.reset_world_settings(client_timed_out=self._client_timed_out)
    ...
    def _load_and_wait_for_world(self, args, town):
        ...
        world_load = self._sim_backend.load_world(town)
        self.world = world_load.world
    ...
    def run(self, args):
        ...
        if crashed and self._sim_backend is not None:
            self._sim_backend.stop_server()
...
def main():
    ...
    parser.add_argument('--server-warmup-seconds',
                        default=float(os.environ.get('SERVER_WARMUP_SECONDS', 30.0)),
                        type=float,
                        help='Seconds to wait after launching the CARLA server before connecting')
    parser.add_argument('--bootstrap-timeout-cap',
                        default=float(os.environ.get('BOOTSTRAP_TIMEOUT_CAP', os.environ.get('TIMEOUT', 600.0))),
                        type=float,
                        help='Per-attempt timeout cap while bootstrapping the CARLA client connection')
