...
if [[ -z "${CARLA_ROOT}" ]]; then
    echo "CARLA_ROOT is required before launching shard4." >&2
    exit 2
fi
if [[ -z "${ADAPTDRIVE_RUN_ROOT}" || "${ADAPTDRIVE_RUN_ROOT}" != /* ]]; then
    echo "Set ADAPTDRIVE_RUN_ROOT to an absolute external run directory before launching shard4." >&2
    exit 2
fi
if [[ -z "${EXPERIMENT_ID}" ]]; then
    echo "Set EXPERIMENT_ID before launching shard4." >&2
    exit 2
fi
if [[ -z "${FINETUNE_CKPT}" ]]; then
    echo "Set FINETUNE_CKPT to the SAC adapter checkpoint before launching shard4." >&2
    exit 2
fi

ROUTES="${ROUTES:-${HIPAD_ROOT}/bench2drive/leaderboard/data/bench2drive220.xml}"
ROUTES_SUBSET="${ROUTES_SUBSET_OVERRIDE:-${ROUTES_SUBSET:-24240-26393}}"
EVALUATION_SHARD="shard4"
...
GPU_RANK="${GPU_RANK:-0}"
PORT="${PORT:-30890}"
TM_PORT="${TM_PORT:-52890}"
export GPU_RANK PORT TM_PORT
export RUN_NAME="${RUN_NAME:-${EXPERIMENT_ID}_shard4}"
export XDG_RUNTIME_DIR="${XDG_RUNTIME_DIR:-/tmp/carla-runtime-hipad-adapter-shard4-gpu${GPU_RANK}}"
export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/matplotlib-hipad-adapter-shard4-gpu${GPU_RANK}}"

echo "[adapter-shard4] GPU=${GPU_RANK} routes=${ROUTES_SUBSET} resume=${RESUME} save=${SAVE_PATH}"
