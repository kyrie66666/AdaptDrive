...
if [[ -z "${CARLA_ROOT}" ]]; then
    echo "CARLA_ROOT is required before launching shard3." >&2
    exit 2
fi
if [[ -z "${ADAPTDRIVE_RUN_ROOT}" || "${ADAPTDRIVE_RUN_ROOT}" != /* ]]; then
    echo "Set ADAPTDRIVE_RUN_ROOT to an absolute external run directory before launching shard3." >&2
    exit 2
fi
if [[ -z "${EXPERIMENT_ID}" ]]; then
    echo "Set EXPERIMENT_ID before launching shard3." >&2
    exit 2
fi
if [[ -z "${FINETUNE_CKPT}" ]]; then
    echo "Set FINETUNE_CKPT to the SAC adapter checkpoint before launching shard3." >&2
    exit 2
fi

ROUTES="${ROUTES:-${HIPAD_ROOT}/bench2drive/leaderboard/data/bench2drive220.xml}"
ROUTES_SUBSET="${ROUTES_SUBSET_OVERRIDE:-${ROUTES_SUBSET:-11715-24224}}"
EVALUATION_SHARD="shard3"
...
GPU_RANK="${GPU_RANK:-1}"
PORT="${PORT:-30790}"
TM_PORT="${TM_PORT:-52790}"
export GPU_RANK PORT TM_PORT
export RUN_NAME="${RUN_NAME:-${EXPERIMENT_ID}_shard3}"
export XDG_RUNTIME_DIR="${XDG_RUNTIME_DIR:-/tmp/carla-runtime-hipad-adapter-shard3-gpu${GPU_RANK}}"
export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/matplotlib-hipad-adapter-shard3-gpu${GPU_RANK}}"

echo "[adapter-shard3] GPU=${GPU_RANK} routes=${ROUTES_SUBSET} resume=${RESUME} save=${SAVE_PATH}"
