...
if [[ -z "${CARLA_ROOT}" ]]; then
    echo "CARLA_ROOT is required before launching shard2." >&2
    exit 2
fi
if [[ -z "${ADAPTDRIVE_RUN_ROOT}" || "${ADAPTDRIVE_RUN_ROOT}" != /* ]]; then
    echo "Set ADAPTDRIVE_RUN_ROOT to an absolute external run directory before launching shard2." >&2
    exit 2
fi
if [[ -z "${EXPERIMENT_ID}" ]]; then
    echo "Set EXPERIMENT_ID before launching shard2." >&2
    exit 2
fi
if [[ -z "${FINETUNE_CKPT}" ]]; then
    echo "Set FINETUNE_CKPT to the SAC adapter checkpoint before launching shard2." >&2
    exit 2
fi

ROUTES="${ROUTES:-${HIPAD_ROOT}/bench2drive/leaderboard/data/bench2drive220.xml}"
ROUTES_SUBSET="${ROUTES_SUBSET_OVERRIDE:-${ROUTES_SUBSET:-3472-11381}}"
EVALUATION_SHARD="shard2"
...
GPU_RANK="${GPU_RANK:-1}"
PORT="${PORT:-30690}"
TM_PORT="${TM_PORT:-52690}"
export GPU_RANK PORT TM_PORT
export RUN_NAME="${RUN_NAME:-${EXPERIMENT_ID}_shard2}"
export XDG_RUNTIME_DIR="${XDG_RUNTIME_DIR:-/tmp/carla-runtime-hipad-adapter-shard2-gpu${GPU_RANK}}"
export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/matplotlib-hipad-adapter-shard2-gpu${GPU_RANK}}"

echo "[adapter-shard2] GPU=${GPU_RANK} routes=${ROUTES_SUBSET} resume=${RESUME} save=${SAVE_PATH}"
